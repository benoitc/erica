...
create(_Args, _Config) ->
    ...
    %% Note that if you define the same template in both ~/.erica/templates
    ...
    Context1 = case erica_config:get_global(template_vars, undefined) of
                   undefined ->
                       Context0;
                   File ->
    ...
    Force = erica_config:get_global(force, "0"),
...
template_id() ->
    case erica_config:get_global(template, undefined) of
        undefined ->
...
find_other_templates() ->
    case erica_config:get_global(template_dir, undefined) of
        undefined ->
            [];
        TemplateDir ->
            erica_util:find_files(TemplateDir, ?TEMPLATE_RE)
    end.
...
update_vars([Key | Rest], Dict) ->
    Value = erica_config:get_global(Key, dict:fetch(Key, Dict)),
